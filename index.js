...
@media (max-width: 480px) {
    ...
    /* Ball unlock highlight animation */
    .ball-just-unlocked {
...
// Show confirmation dialog when user tries to leave/reload the page
// Only for guests who have made progress (not just fresh page load)
let hasGameProgress = false;

function updateGameProgress() {
    // Mark that the user has made some game progress
    hasGameProgress = true;
}

// Check for unsaved progress before page unload
window.addEventListener('beforeunload', (e) => {
    // Only show confirmation if:
    // 1. User is playing as guest
    // 2. Has made some game progress
    // 3. Hasn't saved recently (within last 5 seconds)
    if (isGuest && hasGameProgress) {
        const timeSinceLastSave = window.lastSaveTime ? (Date.now() - window.lastSaveTime) / 1000 : Infinity;

        if (timeSinceLastSave > 5) {
            // Show confirmation dialog
            const confirmationMessage = 'You have unsaved progress! Your game is saved locally, but consider logging in to save to the cloud. Are you sure you want to leave?';
            e.preventDefault();
...
// ============================================
// BALL UNLOCK TRACKING SYSTEM
// ============================================
// Explicitly track which ball types the user has unlocked
// This is separate from ownership (ballCounts) - unlocks determine what's shown in shop
let ballsUnlocked = {
    basic: true,    // Always unlocked by default
    sniper: false,
    big: false,
    explosion: false,
    multiplying: false,
...
/**
 * Unlock a ball type and show notification
 */
function unlockBall(type) {
    // Use ballDisplayNames from later in the file
    const displayNames = {
        basic: 'Basic',
        sniper: 'Sniper',
        big: 'Big',
    ...
    if (!ballsUnlocked[type]) {
        ballsUnlocked[type] = true;
        
        // Show unlock notification
        const displayName = displayNames[type] || type;
        showToast(`🎉 ${displayName} Ball unlocked! Check the shop!`);
        
        // Trigger visual highlight on the shop option
        const optionEl = document.getElementById(`${type}-option`);
        if (optionEl) {
            optionEl.classList.add('ball-just-unlocked');
            setTimeout(() => {
                optionEl.classList.remove('ball-just-unlocked');
            }, 3000);
        }
...
/**
 * Get list of unlocked ball types
 */
function getUnlockedBalls() {
    return Object.entries(ballsUnlocked)
        .filter(([_, unlocked]) => unlocked)
        .map(([type, _]) => type);
}

/**
 * Get list of locked ball types
 */
function getLockedBalls() {
    return Object.entries(ballsUnlocked)
        .filter(([_, unlocked]) => !unlocked)
        .map(([type, _]) => type);
}
...
// ============================================
// END BALL UNLOCK TRACKING SYSTEM
// ============================================
...
// ============================================
// NEW USER EXPERIENCE - PROGRESSIVE UNLOCKS
// ============================================

// Progressive unlock order - new users only see balls in this order
// A ball becomes visible only after owning the previous ball
const ballUnlockOrder = [
    'basic',       // Always visible
    'sniper',      // Unlocks after buying Basic
...
const FEATURE_UNLOCKS = {
    SETS_BUTTON: {
        condition: () => rebirthCount >= 100 && !isGuest,
        elementId: 'custom-option'
    },
    MILESTONES: {
        condition: () => level >= 5 || completedMilestones.length > 0,
        elementId: 'milestones-container'
    }
};

/**
 * Check if a ball should be visible in the shop
 * Uses progressive unlock system - need to own previous ball type first
 */
function isBallVisible(type) {
    const index = ballUnlockOrder.indexOf(type);
    if (index === -1) return true; // Unknown ball types always visible

    if (index === 0) return true; // Basic is always visible

    // Check if previous ball in the sequence is owned
    const previousBall = ballUnlockOrder[index - 1];
    return ballCounts[previousBall] > 0;
}

/**
 * Get the number of different ball types owned
 */
function getOwnedBallTypesCount() {
    return Object.values(ballCounts).filter(count => count > 0).length;
}

/**
 * Show a previously hidden element (with fade-in animation)
 */
function showElement(elementId, animate = true) {
    const element = document.getElementById(elementId);
    if (!element) return false;

    if (element.classList.contains('hidden-for-new-users')) {
        element.classList.remove('hidden-for-new-users');
        if (animate) {
...
/**
 * Hide an element for new users
 */
function hideElement(elementId) {
    const element = document.getElementById(elementId);
    if (!element) return;
    element.classList.add('hidden-for-new-users');
}

/**
 * Check and update feature visibility based on unlock conditions
 */
function updateFeatureVisibility() {
    Object.values(FEATURE_UNLOCKS).forEach(feature => {
        if (feature.condition()) {
            showElement(feature.elementId);
        } else {
            hideElement(feature.elementId);
        }
    });
}

/**
 * Check if milestones section should be shown
 */
function shouldShowMilestones() {
    return level >= 5 || completedMilestones.length > 0;
}

/**
 * Initialize UI for new users
 */
function initializeNewUserUI() {
    updateBallOptionsUI();
    updateFeatureVisibility();

    // Update milestones visibility
    const milestonesContainer = document.getElementById('milestones-container');
    if (milestonesContainer) {
        if (shouldShowMilestones()) {
            milestonesContainer.classList.remove('hidden-for-new-users');
        } else {
            milestonesContainer.classList.add('hidden-for-new-users');
        }
    }
}
...
function isBallUnlocked(type) {
    // First check if ball is explicitly unlocked
    if (ballsUnlocked[type] === true) {
        return true;
    }
    
    // For progressive unlock system: unlock if previous ball type is owned
    // This allows unlocking by purchasing previous balls
    const index = ballUnlockOrder.indexOf(type);
    if (index > 0) {
        const previousBall = ballUnlockOrder[index - 1];
        if (ballCounts[previousBall] > 0) {
            // Auto-unlock the next ball in sequence when previous is owned
            unlockBall(type);
            return true;
        }
    }
...
function updateBallOptionsUI() {
    ...
    // Update price progress bars based on current gold
    updatePriceProgress();
}

/**
 * Update the price progress bars to show how much gold the user has
 * relative to each ball's price. Green fill slides in from left to right.
 */
function updatePriceProgress() {
    // Define all purchasable items with their prices
    const purchasableItems = [
        { id: 'basic', price: ballPrices.basic },
        { id: 'sniper', price: ballPrices.sniper },
        { id: 'big', price: ballPrices.big },
        { id: 'explosion', price: ballPrices.explosion },
        { id: 'multiplying', price: ballPrices.multiplying },
        { id: 'auto', price: autoBallPrice },
        { id: 'cannonball', price: ballPrices.cannonball },
        { id: 'poison', price: ballPrices.poison },
        { id: 'snowball', price: ballPrices.snowball },
        { id: 'weapons', price: 1000 } // Mass Destruction
    ];

    purchasableItems.forEach(item => {
        const optionEl = document.getElementById(item.id === 'auto' ? 'auto-option' :
            item.id === 'weapons' ? 'weapons-btn' :
                `${item.id}-option`);
        if (!optionEl) return;

        const progressBar = optionEl.querySelector('.price-progress');
        if (!progressBar) return;

        // Calculate percentage (capped at 100%)
        const percentage = Math.min((gold / item.price) * 100, 100);
        progressBar.style.width = `${percentage}%`;

        // Add/remove cannot-afford styling
        if (gold < item.price) {
            optionEl.classList.add('cannot-afford');
        } else {
            optionEl.classList.remove('cannot-afford');
        }
    });
}
...
window.onload = function () {
    auth.onAuthStateChanged(user => {
        ...
        if (user && !isGuest) {
            ...
            // Auto-play as guest - don't show login modal
            // If user wants to login, they can click the save button
            playAsGuest();
...
function login() {
    ...
    auth.signInWithEmailAndPassword(email, password)
        .then(userCredential => {
            ...
            // Check for local guest progress and migrate if exists
            const localData = JSON.parse(localStorage.getItem('brickerGameState'));
            
            if (localData) {
                // Merge local guest progress with Firebase data
                // Local progress takes precedence for game state, but we keep Firebase username
                database.ref('users/' + userCredential.user.uid + '/gameState').once('value')
                    .then(snapshot => {
                        const firebaseData = snapshot.val();
                        
                        // Create merged game state - local data with Firebase username
                        const mergedGameState = {
                            ...localData,
                            username: firebaseData?.username || localData.username || 'Player'
                        };
                        
                        // Save merged state to Firebase
                        return database.ref('users/' + userCredential.user.uid + '/gameState').set(mergedGameState);
                    })
                    .then(() => {
                    ...
                    .catch(error => {
                        ...
                        // If migration fails, just load from Firebase
                        loadGameState(userCredential.user.uid);
                        setTimeout(() => location.reload(), 800);
                    });
            } else {
...
function loginWithGoogle() {
    ...
    auth.signInWithPopup(provider)
        .then(result => {
            ...
            if (localData) {
                // Merge local guest progress with Firebase data
                database.ref('users/' + result.user.uid + '/gameState').once('value')
                    .then(snapshot => {
                        const firebaseData = snapshot.val();
                        ...
                        const mergedGameState = {
                            ...localData,
                            username: firebaseData?.username || localData.username || result.additionalUserInfo?.isNewUser ? generateAnonymousUsername() : 'Player'
                        };
                        
                        // Save merged state to Firebase
                        return database.ref('users/' + result.user.uid + '/gameState').set(mergedGameState);
                    })
                    .then(() => {
                        // Clear localStorage after successful migration
                        localStorage.removeItem('brickerGameState');
                        console.log('Guest progress migrated successfully via Google!');
                        showToast('Progress loaded successfully!');
...
function signUp() {
    ...
    auth.createUserWithEmailAndPassword(email, password)
        .then(userCredential => {
            ...
            // Migrate guest progress from localStorage to new account
            const localData = JSON.parse(localStorage.getItem('brickerGameState')) || {};
            localData.username = newUsername;

            // Transfer guest progress to Firebase
            database.ref('users/' + userCredential.user.uid + '/gameState').set(localData)
                .then(() => {
                    // Clear localStorage after successful migration
                    localStorage.removeItem('brickerGameState');
...
function playAsGuest() {
    ...
    // Load full game state from localStorage if available
    const localData = JSON.parse(localStorage.getItem('brickerGameState'));
    if (localData) {
        // Restore all game state for returning guests
        gold = localData.gold || 20;
        level = localData.level || 1;
        idleGoldPerSecond = localData.idleGoldPerSecond || 0.5;
        rebirthCount = localData.rebirthCount || 0;
        autoRebirthOwned = localData.autoRebirthOwned || false;
        autoRebirthEnabled = !!localData.autoRebirthEnabled;
        
        // Restore ball counts - ensure ALL ball types are included
        ballCounts = {
            basic: localData.ballCounts?.basic ?? 0,
            sniper: localData.ballCounts?.sniper ?? 0,
            big: localData.ballCounts?.big ?? 0,
            explosion: localData.ballCounts?.explosion ?? 0,
            multiplying: localData.ballCounts?.multiplying ?? 0,
            child: localData.ballCounts?.child ?? 0,
            cannonball: localData.ballCounts?.cannonball ?? 0,
            poison: localData.ballCounts?.poison ?? 0,
            snowball: localData.ballCounts?.snowball ?? 0
        };
        ...
        // Restore ball upgrades
        ballUpgrades = localData.ballUpgrades || ballUpgrades;
        
        // Restore stats
        stats = localData.stats || stats;
        
        // Restore cosmetics and mascots
        cosmetics = localData.cosmetics || cosmetics;
        mascotsOwned = Array.isArray(localData.mascotsOwned) ? localData.mascotsOwned : (localData.mascotsOwned || {});
        selectedMascotId = localData.selectedMascotId || null;
        
        // Restore milestones and rewards
        completedMilestones = Array.isArray(localData.completedMilestones) ? localData.completedMilestones : [];
        unclaimedMilestoneRewards = localData.unclaimedMilestoneRewards || {};
        
        // Restore global upgrades
        if (localData.globalUpgrades) {
            globalUpgrades = localData.globalUpgrades;
        }
        
        // Restore ball unlock tracking (with fallback for existing guests)
        if (localData.ballsUnlocked) {
            ballsUnlocked = { ...ballsUnlocked, ...localData.ballsUnlocked };
        }
        // Ensure basic ball is always unlocked
        ballsUnlocked.basic = true;

        // CRITICAL: Update ball options UI immediately to show unlocked balls
        // This must be called BEFORE startGame() to ensure correct display on reload
        updateBallOptionsUI();
    }
...
function saveGameState() {
    if (isGuest) {
        ...
        saveToLocalStorage();
        // Update the "last saved" timestamp for beforeunload confirmation
    ...
    const user = auth.currentUser;
    if (user) {
        // Build ballCounts from the balls array - includes ALL ball types
        const ballCountsToSave = {
        ...
        const gameState = {
            ...
            stats: {
                ...stats,
                totalBricksBroken: stats.totalBricksBroken || 0
            },
            username,
            rebirthCount,
            cosmetics,
            ...
            mascotsOwned,
            selectedMascotId,
            ballsUnlocked,  // Save ball unlock tracking
            settings: {
...
let unclaimedMilestoneRewards = {}; // Track unclaimed rewards by milestone ID
...
function renderMilestones() {
  ...
  <div class="milestone-progress">
    ...
    // Remove any existing unclaimed rewards container to avoid duplicates
    const existingUnclaimedContainer = container.parentElement?.querySelector('#unclaimed-rewards-container');
    if (existingUnclaimedContainer) {
        existingUnclaimedContainer.remove();
    }
    
    // Show unclaimed rewards
    if (Object.keys(unclaimedMilestoneRewards).length > 0) {
        const unclaimedContainer = document.createElement('div');
        unclaimedContainer.id = 'unclaimed-rewards-container';
        Object.entries(unclaimedMilestoneRewards).forEach(([milestoneId, reward]) => {
            const milestone = milestoneDefinitions.find(m => m.id === milestoneId);
            if (!milestone) return;
            
            const rewardEl = document.createElement('div');
            rewardEl.className = 'unclaimed-milestone-reward';
            rewardEl.innerHTML = `
...
function updateMilestones() {
    ...
    [...activeMilestones].forEach(m => {
        if (getStatValue(m.stat) >= m.goal) {
            ...
            if (!completedMilestones.includes(m.id)) {
                ...
                if (m.reward) {
                    unclaimedMilestoneRewards[m.id] = m.reward;
                }
            }

            // Show a toast notification (non-blocking)
            showToast(`🏆 Milestone Complete: ${m.title}! Claim your reward!`);
    ...
    // Save completed milestones and unclaimed rewards to Firebase if any new ones
...
let currentLeaders = [];
let previousRanks = {};
let rankChangeQueue = null; // Track pending rank change notifications
let rankChangeTimeout = null; // Timeout for debouncing rank changes
...
function updateLeaderboard() {
    ...
    database.ref('users').once('value')
        .then(snapshot => {
            ...
            newLeaders.forEach((leader, index) => {
                ...
                // 🔥 Detect rank improvement (only show latest/best rank change)
                if (oldRank && newRank < oldRank) {
                    // Store only the latest rank change (highest improvement)
                    if (!rankChangeQueue || newRank < rankChangeQueue.newRank) {
                        rankChangeQueue = { username: leader.username, newRank: newRank };
                    }
                }
    ...
    `;
            ...
            // Show the queued rank change toast (only 1 at a time, the latest/best)
            if (rankChangeQueue) {
                // Clear existing timeout
                if (rankChangeTimeout) {
                    clearTimeout(rankChangeTimeout);
                }
                // Remove any existing rank change toasts
                const existingRankToasts = document.querySelectorAll('.newranktoast');
                existingRankToasts.forEach(toast => toast.remove());
                
                showNewRankToast(rankChangeQueue.username, rankChangeQueue.newRank);
                rankChangeQueue = null; // Reset for next update
            }
    ...
    database.ref('users/' + uid + '/gameState').once('value')
        .then((snapshot) => {
            ...
            if (data) {
                ...
                // Load completed milestones and unclaimed rewards
                completedMilestones = Array.isArray(data.completedMilestones) ? data.completedMilestones : [];
                unclaimedMilestoneRewards = data.unclaimedMilestoneRewards || {};

                // Load ball unlock tracking (with fallback for existing players)
                if (data.ballsUnlocked) {
                    ballsUnlocked = { ...ballsUnlocked, ...data.ballsUnlocked };
                }
                // Ensure basic ball is always unlocked
                ballsUnlocked.basic = true;
...
function startGame() {
    if (!gameStarted) {
        ...
        // Initialize new user UI - hide elements until unlocked
        initializeNewUserUI();
...
// Ball type display names for notifications
const ballDisplayNames = {
    basic: 'Basic',
    sniper: 'Sniper',
    big: 'Big',
    explosion: 'Bomb',
...
/**
 * Check if a ball type was just unlocked and show notification
 * Returns the name of the newly unlocked ball type if any, null otherwise
 */
function checkForNewBallUnlock() {
    // Check each ball in the unlock order (skip first which is always visible)
    for (let i = 1; i < ballUnlockOrder.length; i++) {
        const ballType = ballUnlockOrder[i];
        const previousBall = ballUnlockOrder[i - 1];
        
        // Check if previous ball is now owned (count just became > 0)
        // and the current ball wasn't visible before
        if (ballCounts[previousBall] > 0) {
            // The next ball in the sequence should now be visible
            const nextBall = ballUnlockOrder[i];
            const optionEl = document.getElementById(`${nextBall}-option`);
            
            if (optionEl) {
                // Check if element was previously hidden (display: none)
                const wasHidden = optionEl.style.display === 'none' || 
                                  optionEl.classList.contains('ball-hidden');
                
                if (wasHidden) {
                    // Ball was just unlocked!
                    ...
                    return ballDisplayNames[nextBall] || nextBall;
                }
            }
        }
    }
...
function buyBall(type) {
    if (gold >= ballPrices[type] && balls.length < maxBalls) {
        ...
        const newlyUnlocked = checkForNewBallUnlock();
        if (newlyUnlocked) {
            showToast(`🎉 ${newlyUnlocked} Ball unlocked! Check the shop scroll!`);
        }
    } else {
...
function upgradeBall(type) {
    ...
    // Max upgrade level: scales with rebirths
...
function showMyBallsPopup() {
    ...
    // Generate ball stats HTML
    const ballTypes = [
        { type: 'basic', name: 'Basic', cost: basicCost, color: 'gray', radius: 8 },
        { type: 'sniper', name: 'Sniper', cost: sniperCost, color: 'blue', radius: 8 },
        { type: 'big', name: 'Big', cost: bigCost, color: 'green', radius: 12 },
        { type: 'explosion', name: 'Bomb', cost: explosionCost, color: 'orange', radius: 8 },
        { type: 'multiplying', name: 'Clones', cost: multiplyingCost, color: 'purple', radius: 8 },
        { type: 'cannonball', name: 'Cannon', cost: cannonballCost, color: 'darkgray', radius: 8 },
        { type: 'poison', name: 'Poison', cost: poisonCost, color: 'lime', radius: 8 },
        { type: 'snowball', name: 'Snowball', cost: snowballCost, color: 'white', radius: 8 }
    ];

    let ballStatsHTML = '';
    ballTypes.forEach(ball => {
        if (isBallVisible(ball.type)) {
            const ballCount = balls.filter(b => b.type === ball.type).length;
            const canUpgrade = gold >= ball.cost && ballUpgrades[ball.type].level < maxUpgradeLevel;
            ballStatsHTML += `
                <div class="ball-stat-row" data-ball-type="${ball.type}">
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <canvas id="stats-${ball.type}-preview" width="40" height="40"></canvas>
                        <span><b>${ball.name}</b>: ${ballCount} balls, Level: ${ballUpgrades[ball.type].level}/${maxUpgradeLevel}, Speed: ${ballUpgrades[ball.type].speed.toFixed(1)}, Damage: ${formatNumber(ballUpgrades[ball.type].damage.toFixed(1))}</span>
                    </div>
                    <button class="upgrade-btn ${canUpgrade ? '' : 'disabled'}" data-action="upgrade" data-type="${ball.type}">Upgrade<br>(${formatNumber(ball.cost)})</button>
                </div>
            `;
        }
    });
    ...
    // Auto balls section
    ballStatsHTML += `
        <div class="ball-stat-row">
            <div style="display: flex; align-items: center; gap: 10px;">
                <canvas id="stats-auto-preview" width="40" height="40"></canvas>
        ...
        <div class="my-balls-popup">
            ...
            <div class="tab-content" id="upgrades">
                <div class="upgrades-list">
                    <div class="upgrade-row">
                        ...
                        <span class="upgrade-desc">Speed Boost: Increase all ball speed by 2x for 1 minute</span>
                        <button class="upgrade-btn ${globalUpgrades.speedBoost.active || gold < globalUpgrades.speedBoost.cost ? 'disabled' : ''}" data-action="global-upgrade" data-type="speedBoost">${globalUpgrades.speedBoost.active ? 'Active' : 'Activate'}<br>(${globalUpgrades.speedBoost.cost} gold)</button>
                    </div>
                    <div class="upgrade-row">
                        <div class="upgrade-icon">💰</div>
                        <span class="upgrade-desc">Gold Boost: Increase gold production by 2x for 1 minute</span>
                        <button class="upgrade-btn ${globalUpgrades.goldBoost.active || gold < globalUpgrades.goldBoost.cost ? 'disabled' : ''}" data-action="global-upgrade" data-type="goldBoost">${globalUpgrades.goldBoost.active ? 'Active' : 'Activate'}<br>(${globalUpgrades.goldBoost.cost} gold)</button>
                    </div>
                </div>
            </div>
            <div class="tab-content" id="rebirth">
                <div class="rebirth-info">
                    ...
                    <p>Each rebirth increases idle gold per second by 1.2 times and max upgrade level by 2.</p>
                    <button class="rebirth-btn ${level < 100 ? 'disabled' : ''}" data-action="rebirth">Rebirth (Level ${level}/100)</button>
                </div>
            </div>
            <div class="tab-content" id="shop">
                ...
                <div class="shop-options">
                    ...
                    <div class="shop-option">
                        ...
                        <button data-action="buy-powerup" data-cost="1" ${rebirthCount < 1 ? 'disabled' : ''}>Buy (1 Rebirth)</button>
                    </div>
                    <div class="shop-option">
                        <span>💰 Idle Gold Boost</span>
                        <span class="tooltip">Permanently increases idle gold by +0.5/sec.</span>
                        <button data-action="buy-idle-boost" ${gold < Math.floor(1500 * Math.max(1, Math.floor(idleGoldPerSecond / 0.5)) * 1.5 * (0.5 / (idleGoldPerSecond * 0.2 || 0.1))) ? 'disabled' : ''}>Buy (${formatNumber(Math.floor(1500 * Math.max(1, Math.floor(idleGoldPerSecond / 0.5)) * 1.5 * (0.5 / (idleGoldPerSecond * 0.2 || 0.1))))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>🌌 Cosmic Barrage</span>
                        <span class="tooltip">Destroys 15 bricks and triples ball speed for 30 seconds.</span>
                        <button data-action="buy-cosmic" data-cost="5" ${rebirthCount < 5 ? 'disabled' : ''}>Buy (5 Rebirths)</button>
                    </div>
                    <h3>Skins & Trails</h3>
                    <div class="shop-option">
                        <span>🌌 Nebula Ball Skin</span>
                        <span class="tooltip">Cosmic-themed ball skin with a starry gradient.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="nebulaSkin" data-cost="10rebirths" ${!cosmetics.nebulaSkin && rebirthCount < 10 ? 'disabled' : ''}>${cosmetics.nebulaSkin ? 'Delete' : 'Buy (10 Rebirths)'}</button>
                    </div>
                    <div class="shop-option">
                        <span>🔥 Fire Trail</span>
                        <span class="tooltip">Leaves a fiery trail behind balls.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="fireTrail" data-cost="500gold" ${!cosmetics.fireTrail && gold < 500 ? 'disabled' : ''}>${cosmetics.fireTrail ? 'Delete' : 'Buy (500 Gold)'}</button>
                    </div>
                    <div class="shop-option">
                        <span>❄️ Ice Trail</span>
                        <span class="tooltip">Leaves a frosty trail behind balls.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="iceTrail" data-cost="500gold" ${!cosmetics.iceTrail && gold < 500 ? 'disabled' : ''}>${cosmetics.iceTrail ? 'Delete' : 'Buy (500 Gold)'}</button>
                    </div>
                    <div class="shop-option">
                        <span>🌈 Rainbow Ball Skin</span>
                        <span class="tooltip">Vibrant, color-shifting ball skin.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="rainbowSkin" data-cost="15rebirths" ${!cosmetics.rainbowSkin && rebirthCount < 15 ? 'disabled' : ''}>${cosmetics.rainbowSkin ? 'Delete' : 'Buy (15 Rebirths)'}</button>
                    </div>
                    <h3>Bundles</h3>
                    <div class="shop-option">
                        <span>🥈 Silver Bundle</span>
                        <span class="tooltip">Includes Nebula Skin and 2 Random Powerups.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="bundleSilver" data-cost="20rebirths" ${!cosmetics.bundleSilver && rebirthCount < 20 ? 'disabled' : ''}>${cosmetics.bundleSilver ? 'Delete' : 'Buy (20 Rebirths)'}</button>
                    </div>
                    <div class="shop-option">
                        <span>🥇 Gold Bundle</span>
                        <span class="tooltip">Includes Nebula Skin, Fire Trail, and 3 Random Powerups.</span>
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="bundleGold" data-cost="30rebirths" ${!cosmetics.bundleGold && rebirthCount < 30 ? 'disabled' : ''}>${cosmetics.bundleGold ? 'Delete' : 'Buy (30 Rebirths)'}</button>
                    </div>
                    <div class="shop-option">
                        ...
                        <button class="cosmetic-btn" data-action="cosmetic" data-cosmetic="bundlePlatinum" data-cost="50rebirths" ${!cosmetics.bundlePlatinum && rebirthCount < 50 ? 'disabled' : ''}>${cosmetics.bundlePlatinum ? 'Delete' : 'Buy (50 Rebirths)'}</button>
                    </div>
                    <h3>Boosts</h3>
                    <div class="shop-option">
                        <span>💥 Instant Brick Destroy</span>
                        <span class="tooltip">Destroys 1 random brick instantly.</span>
                        <button data-action="buy-instant-destroy" data-price="${getDynamicPrice(100)}">Buy (${formatNumber(getDynamicPrice(100))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>⚔️ Double Damage</span>
                        <span class="tooltip">Doubles ball damage for 30 seconds.</span>
                        <button data-action="buy-double-damage" data-price="${getDynamicPrice(150)}">Buy (${formatNumber(getDynamicPrice(150))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>⚡ Extra Ball</span>
                        <span class="tooltip">Spawns an extra ball instantly.</span>
                        <button data-action="buy-extra-ball" data-price="${getDynamicPrice(200)}">Buy (${formatNumber(getDynamicPrice(200))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>🧲 Gold Magnet</span>
                        <span class="tooltip">Collects 2x gold from bricks for 30 seconds.</span>
                        <button data-action="buy-gold-magnet" data-price="${getDynamicPrice(250)}">Buy (${formatNumber(getDynamicPrice(250))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>💪 Brick Health Reduction</span>
                        <span class="tooltip">Reduces all brick health by 15% for 30 seconds.</span>
                        <button data-action="buy-health-reduction" data-price="${getDynamicPrice(300)}">Buy (${formatNumber(getDynamicPrice(300))} Gold)</button>
                    </div>
                    <div class="shop-option">
                        <span>💨 Speed Surge</span>
                        <span class="tooltip">Increases ball speed by 50% for 30 seconds.</span>
                        <button data-action="buy-speed-surge" data-price="${getDynamicPrice(350)}">Buy (${formatNumber(getDynamicPrice(350))} Gold)</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    ballTypes.forEach(ball => {
        if (isBallVisible(ball.type)) {
            const canvas = overlay.querySelector(`#stats-${ball.type}-preview`);
            if (canvas) drawBallPreviewOnCanvas(canvas, ball.color, ball.radius);
        }
    });
    const autoCanvas = overlay.querySelector('#stats-auto-preview');
    if (autoCanvas) drawBallPreviewOnCanvas(autoCanvas, 'black', 8);
    ...
    // Upgrade buttons in My Balls tab
    overlay.querySelectorAll('[data-action="upgrade"]').forEach(btn => {
        btn.addEventListener('click', () => {
            const type = btn.dataset.type;
            if (type) {
                upgradeBall(type);
                // Refresh the popup after upgrade
                overlay.remove();
                showMyBallsPopup();
            }
        });
    ...
    // Global upgrade buttons
    overlay.querySelectorAll('[data-action="global-upgrade"]').forEach(btn => {
        btn.addEventListener('click', () => {
            const type = btn.dataset.type;
            if (type) {
                applyGlobalUpgrade(type);
                // Refresh the popup after upgrade
                overlay.remove();
                showMyBallsPopup();
            }
    ...
    // Rebirth button
    const rebirthBtn = overlay.querySelector('[data-action="rebirth"]');
    if (rebirthBtn) {
    ...
    // Show auto rebirth toggle if owned
    const autoToggleContainer = document.getElementById('auto-rebirth-toggle-container');
    if (autoToggleContainer) {
        autoToggleContainer.style.display = autoRebirthOwned ? 'block' : 'none';
    }

    // Shop buttons
    overlay.querySelectorAll('[data-action="buy-powerup"]').forEach(btn => {
        btn.addEventListener('click', () => {
    ...
    overlay.querySelectorAll('[data-action="buy-idle-boost"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyIdleGoldBoost();
            overlay.remove();
            showMyBallsPopup();
    ...
    overlay.querySelectorAll('[data-action="buy-cosmic"]').forEach(btn => {
        btn.addEventListener('click', () => {
            const cost = parseInt(btn.dataset.cost) || 5;
            buyPowerup(cost, 1, 'cosmicBarrage');
            overlay.remove();
            showMyBallsPopup();
        });
    });

    overlay.querySelectorAll('[data-action="cosmetic"]').forEach(btn => {
        btn.addEventListener('click', () => {
            const cosmetic = btn.dataset.cosmetic;
            if (cosmetic) {
                if (cosmetics[cosmetic]) {
    ...
    overlay.querySelectorAll('[data-action="buy-instant-destroy"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyInstantBrickDestroy();
            overlay.remove();
            showMyBallsPopup();
        });
    });

    overlay.querySelectorAll('[data-action="buy-double-damage"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyDoubleDamage();
            overlay.remove();
            showMyBallsPopup();
    ...
    overlay.querySelectorAll('[data-action="buy-extra-ball"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyExtraBall();
            overlay.remove();
            showMyBallsPopup();
    ...
    overlay.querySelectorAll('[data-action="buy-gold-magnet"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyGoldMagnet();
            overlay.remove();
            showMyBallsPopup();
    ...
    overlay.querySelectorAll('[data-action="buy-health-reduction"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buyBrickHealthReduction();
            overlay.remove();
            showMyBallsPopup();
        });
    });

    overlay.querySelectorAll('[data-action="buy-speed-surge"]').forEach(btn => {
        btn.addEventListener('click', () => {
            buySpeedSurge();
            overlay.remove();
            showMyBallsPopup();
...
// Helper function to draw ball preview on canvas
function drawBallPreviewOnCanvas(canvas, color, radius) {
    if (!canvas) return;
    const ctx = canvas.getContext('2d'); // ← Added quotes around 2d
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.beginPath();
...
// Buy Instant Brick Destroy
function buyInstantBrickDestroy() {
    const price = getDynamicPrice(100);
    if (gold >= price && bricks.length > 0) {
        gold -= price;
        // Destroy 3 random bricks
        for (let i = 0; i < 3 && bricks.length > 0; i++) {
            const randomIndex = Math.floor(Math.random() * bricks.length);
            bricks[randomIndex].health = 0;
            gold += 5;
        }
        updateUI();
        saveGameState();
        showToast("Destroyed 3 bricks! +15 gold");
    } else {
        showToast(gold < price ? "Not enough gold!" : "No bricks to destroy!");
    }
}

// Buy Double Damage
function buyDoubleDamage() {
    const price = getDynamicPrice(150);
    if (gold >= price) {
        gold -= price;
        ballUpgrades.basic.damage *= 2;
        ballUpgrades.sniper.damage *= 2;
        ballUpgrades.big.damage *= 2;
        ballUpgrades.explosion.damage *= 2;
        ballUpgrades.multiplying.damage *= 2;
        ballUpgrades.cannonball.damage *= 2;
        ballUpgrades.poison.damage *= 2;
        ballUpgrades.snowball.damage *= 2;
        setTimeout(() => {
            ballUpgrades.basic.damage /= 2;
            ballUpgrades.sniper.damage /= 2;
            ballUpgrades.big.damage /= 2;
            ...
            ballUpgrades.poison.damage /= 2;
            ballUpgrades.snowball.damage /= 2;
            showToast("Double damage effect ended!");
            updateUI();
        }, 30000);
        updateUI();
        saveGameState();
        showToast("Double damage activated for 30 seconds!");
    } else {
        showToast("Not enough gold!");
    }
}
...
// Buy Extra Ball
function buyExtraBall() {
    const price = getDynamicPrice(200);
    if (gold >= price && balls.length < maxBalls) {
        gold -= price;
        const radius = 8;
        const speed = 3.5;
        balls.push(new Ball(canvas.width / 2, canvas.height / 2, radius, speed * (Math.random() > 0.5 ? 1 : -1), speed * (Math.random() > 0.5 ? 1 : -1), 'gray', 'basic'));
        updateUI();
        saveGameState();
        showToast("Extra ball spawned!");
    } else {
        showToast(balls.length >= maxBalls ? "Max balls reached!" : "Not enough gold!");
    }
}
...
// Buy Gold Magnet
function buyGoldMagnet() {
    const price = getDynamicPrice(250);
    if (gold >= price) {
        gold -= price;
        const originalGoldPerSecond = idleGoldPerSecond;
        idleGoldPerSecond *= 2;
        setTimeout(() => {
            idleGoldPerSecond = originalGoldPerSecond;
            showToast("Gold magnet effect ended!");
            updateUI();
        }, 30000);
        updateUI();
        saveGameState();
        showToast("Gold magnet activated for 30 seconds! (2x gold)");
    } else {
        showToast("Not enough gold!");
    }
}
...
// Buy Brick Health Reduction
function buyBrickHealthReduction() {
    const price = getDynamicPrice(300);
    if (gold >= price) {
        gold -= price;
        bricks.forEach(brick => {
            brick.health = Math.ceil(brick.health * 0.85);
        });
        updateUI();
        saveGameState();
        showToast("Brick health reduced by 15%!");
    } else {
        showToast("Not enough gold!");
    }
}
...
// Buy Speed Surge
function buySpeedSurge() {
    const price = getDynamicPrice(350);
    if (gold >= price) {
        gold -= price;
        ballUpgrades.basic.speed *= 1.5;
        ballUpgrades.sniper.speed *= 1.5;
        ballUpgrades.big.speed *= 1.5;
        ballUpgrades.explosion.speed *= 1.5;
        ballUpgrades.multiplying.speed *= 1.5;
        ballUpgrades.cannonball.speed *= 1.5;
        ballUpgrades.poison.speed *= 1.5;
        ballUpgrades.snowball.speed *= 1.5;
        setTimeout(() => {
            ballUpgrades.basic.speed /= 1.5;
            ballUpgrades.sniper.speed /= 1.5;
            ballUpgrades.big.speed /= 1.5;
            ...
            ballUpgrades.poison.speed /= 1.5;
            ballUpgrades.snowball.speed /= 1.5;
            showToast("Speed surge effect ended!");
            updateUI();
        }, 30000);
        updateUI();
        saveGameState();
        showToast("Speed surge activated for 30 seconds! (1.5x speed)");
    } else {
        showToast("Not enough gold!");
    }
}
...
// Buy Idle Gold Boost
function buyIdleGoldBoost() {
    const price = Math.floor(1500 * Math.max(1, Math.floor(idleGoldPerSecond / 0.5)) * 1.5 * (0.5 / (idleGoldPerSecond * 0.2 || 0.1)));
    if (gold >= price) {
        gold -= price;
        idleGoldPerSecond += 0.5;
        updateUI();
        saveGameState();
        showToast("Idle gold increased to " + formatNumber(idleGoldPerSecond) + "/sec!");
    } else {
        showToast("Not enough gold!");
    }
}
...
function buyCosmetic(cosmeticName) {
    ...
    switch (cosmeticName) {
        case 'nebulaSkin': costRebirths = 10; break;
        case 'fireTrail':
        case 'iceTrail': costGold = 500; break;
        case 'rainbowSkin': costRebirths = 15; break;
        case 'bundleSilver': costRebirths = 20; break;
        case 'bundleGold': costRebirths = 30; break;
        case 'bundlePlatinum': costRebirths = 50; break;
    }

    if (costRebirths > 0 && rebirthCount >= costRebirths) {
        rebirthCount -= costRebirths;
        cosmetics[cosmeticName] = true;
        if (cosmeticName === 'bundleSilver') cosmetics.nebulaSkin = true;
        if (cosmeticName === 'bundleGold') { cosmetics.nebulaSkin = true; cosmetics.fireTrail = true; }
        if (cosmeticName === 'bundlePlatinum') { cosmetics.nebulaSkin = true; cosmetics.fireTrail = true; cosmetics.iceTrail = true; cosmetics.rainbowSkin = true; }
        const user = auth.currentUser;
        if (user) {
            database.ref('users/' + user.uid + '/gameState').update({
                rebirthCount,
    ...
    } else if (costGold > 0 && gold >= costGold) {
        gold -= costGold;
        cosmetics[cosmeticName] = true;
        updateUI();
        saveGameState();
        ...
        showToast(costRebirths > 0 ? "Not enough rebirths!" : "Not enough gold!");
    }
}

function closeCustomBallModal() {
    const modal = document.querySelector('.custom-ball-modal');
    if (modal) modal.remove();
}

// Add event listener to the custom balls button
document.getElementById('custom-option')?.addEventListener('click', showCustomBallModal);
...
// Call startGameLoop when the page loads
